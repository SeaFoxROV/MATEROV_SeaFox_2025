...
class CameraPublisher(Node):
    def __init__(self):
        ...
        # cameras_index = [0, 8, 3, 6]  # adjust as needed this is for the jetson
        cameras_index = [0, 2, 3, 6]  # adjust as needed this is for the jetson
        self.captures = [cv2.VideoCapture(i) for i in cameras_index]
        for i, cap in enumerate(self.captures):
            cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
            cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
            cap.set(cv2.CAP_PROP_FPS, 30)
            #cap = cv2.resize(image, dim, interpolation=cv2.INTER_AREA)
            if not cap.isOpened():
                print()
